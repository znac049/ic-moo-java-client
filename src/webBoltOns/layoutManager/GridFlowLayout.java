package webBoltOns.layoutManager;

/*
 * $Id: GridFlowLayout.Java ,v 1.050725 2005/07/25 08:15:39  jrivet.com    Exp $ $Name:  $
 *
 * Copyright  2003, 2004, 2005  www.jrivet.com
 * 
 *   @author  
 * 	 @version 1.050725
 *
 ...
 *  the Initial Developer are Copyright (C) 2003, 2004, 2005  by Paul Jones.
 *
 *  **All Rights Reserved **.
 *
...
public class GridFlowLayout implements LayoutManager2, Serializable {
	...
	public void addLayoutComponent(Component component, Object constraints) {
		synchronized (component.getTreeLock()) {
			...
			double compWidth = component.getPreferredSize().getWidth(), compHeight = component
			...
			Dimension location = cellSizes.get(cr);
			...
			if (location == null) {
				...
				orow = 0;
				location.setSize(location.getWidth() + compWidth + vgap,
						location.getHeight());
			...
			constraintsTable.put(component, componentConstraint);
	...
	private double[] getColumnPositions() {
		...
		for (int c = 0; c < col; c++) {
			...
			for (int r = 0; r < row; r++) {
				Dimension size = cellSizes.get(new Dimension(
						(c + 1), (r + 1)));
				if (size != null) {
	...
	private double[] getRowPositions() {
		...
		for (int r = 0; r < row; r++) {
			...
			for (int c = 0; c < col; c++) {
				Dimension size = cellSizes.get(new Dimension((c + 1), (r + 1)));
				if (size != null) {
	...
	public void layoutContainer(Container parent) {
		synchronized (parent.getTreeLock()) {
			...
			for (int i = comps.length - 1; i >= 0; i--) {
				...
				ComponentConstraint componentConstraint = constraintsTable
						.get(component);
				if (componentConstraint != null ) {
					...
					if (location != null) {
						...
						component.setBounds(absoluteX, absoluteY,
	...
	private void solveCell(Component component,
		...
		do {
			notComplete = false;
			i++;
			Component nextComponent = componentTable.get(Integer
					.toString(i));
			if (nextComponent != null) {
				ComponentConstraint nextComponentConstraint = constraintsTable
						.get(nextComponent);
				if (nextComponentConstraint.getCellLocation().getHeight() == componentConstraint
					...
					Dimension offset = nextComponentConstraint.getOffset();
		...
		Component comps[] = parent.getComponents();
			for (int i = 0; i < comps.length; i++) {
				...
				if (componentConstraint != null && c != null ) {
	 			...
	 			if (w != 0 || h != 0 ) {
						Dimension d = cellSizes
								.get(componentConstraint.getCellLocation());
						...
						d.setSize(newW + vgap, newH + hgap);
  						if(component.isVisible()) {
  							...
  							cellSizes.put(componentConstraint.getCellLocation(), new Dimension(0,0));
  							componentConstraint.setCurrentSize(new Dimension(0,0));
  						}
					}
				}
			}
